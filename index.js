...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `
            ...
            <h3>${games[i].name}</h3>
            <p>${games[i].description} (Backers: ${games[i].backers})</p>
        `;
...
// Call the function with the parsed GAMES_JSON
...
const unfundedGamesStr = `${unfundedGamesCount} ${unfundedGamesCount === 1 ? 'game remains' : 'games remain'} unfunded.`;

console.log(unfundedGamesStr);


// create a new DOM element containing the template string and append it to the description container
const newDescriptionContainer = document.getElementById("description-container");

newDescriptionContainer.innerHTML = `
    <p>
        The purpose of our company is to fund independent games. We've been in operation for 12 years.
    </p>
    <p>
        A total of $${GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString()} 
        has been raised for ${GAMES_JSON.length} games.
        Currently, ${GAMES_JSON.filter(game => game.pledged < game.goal).length} 
        ${GAMES_JSON.filter(game => game.pledged < game.goal).length === 1 ? 'game remains' : 'games remain'} unfunded.
